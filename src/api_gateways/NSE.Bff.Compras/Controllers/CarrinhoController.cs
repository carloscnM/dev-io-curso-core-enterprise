...
namespace NSE.Bff.Compras.Controllers
{
    ...
    public class CarrinhoController : MainController
    {
        ...
        {
            CarrinhoDTO carrinho = await _carrinhoService.ObterCarrinho();
            return carrinho?.Itens.Sum(item => item.Quantidade) ?? 0;
        }
        ...
        {
            ItemProdutoDTO produto = await _catalogoService.ObterPorId(itemProduto.ProdutoId);
            ...
            ResponseResult resposta = await _carrinhoService.AdicionarItemCarrinho(itemProduto);
        ...
        {
            ItemProdutoDTO produto = await _catalogoService.ObterPorId(produtoId);

            await ValidarItemCarrinho(produto, itemProduto.Quantidade);
            if (!OperacaoValida()) return CustomResponse();

            ResponseResult resposta = await _carrinhoService.AtualizarItemCarrinho(produtoId, itemProduto);
        ...
        public async Task<IActionResult> RemoverItemCarrinho(Guid produtoId)
        {
            ...
            ResponseResult resposta = await _carrinhoService.RemoverItemCarrinho(produtoId);
        ...
        public async Task<IActionResult> AplicarVoucher([FromBody] string voucherCodigo)
        {
            ...
            ResponseResult reposta = await _carrinhoService.AplicarVoucherCarrinho(voucher);

            return CustomResponse(reposta);
        }
